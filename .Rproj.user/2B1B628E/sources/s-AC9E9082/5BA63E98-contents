...
## Replicating sma SS3 model run and diagnostics.
## Step 1) Create a folder on your computer and copy the following files to it: Starter.ss ,  forecast.ss , ss3.exe , CONTROL.ss , DATA.ss
## Step 2) Open terminal and run model without hessian: ss3 -nohess
## Step 3) Create rdata with ss_outputs. Run the seven lines below.

library(r4ss)
library(tseries)
library(reshape2)
setwd("C:/Users/mkapur/Dropbox/UW/coursework/FISH-555/Model_162 - Copy")
...
## Step 4) Run getML.R to append the mean length data to smabase.rdata. - NOT WORKING
## Step 5) Load rdata

load("MLS.rdata",verbose=T)

## Step 6) Run rest of the code
...
rec.est = ss.rec[is.na(ss.rec$dev)==F & ss.rec$dev != 0,]
...
xlim =range(ss.rec$years)
ylim = c(min(-1.22,abs(rec.est$dev)*-1.1),max(1.22,abs(rec.est$dev)*1.1))#range(proc.dev)*1.1
